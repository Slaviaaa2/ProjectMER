...
using System;
using System.IO;
using PMER.Scripts.Modifiers;
using UnityEditor;
using UnityEngine;

/// <summary>
/// Main editor window for managing schematics.
/// Unity 6 compatible (uses FindObjectsByType instead of FindObjectsOfType).
/// </summary>
[InitializeOnLoad]
public class SchematicManager : EditorWindow
{
    ...
    private static void CompileAllButton()
    {
        ...
        {
            if (FindObjectsByType<ModifierBase>(FindObjectsSortMode.None).Length > 0)
            {
    ...
    private static void LogPlayModeState(PlayModeStateChange state)
    {
        ...
        ModifierBase[] modifiers = FindObjectsByType<ModifierBase>(FindObjectsSortMode.None);
    ...
    {
        foreach (Schematic schematic in FindObjectsByType<Schematic>(FindObjectsSortMode.None))
        {
    ...
    private void OnGUI()
    {
        ...
        if (GUI.Button(
            ...
            Config.ExportPath = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.Desktop), "ProjectMER_CompiledSchematics");
